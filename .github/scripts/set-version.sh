...
if [ "$plugin_version" != "$version" ]; then
  echo "Error: failed to update .claude-plugin/plugin.json version to $version (found: ${plugin_version:-<missing>})"
  exit 1
fi
...
if [ "$csproj_version" != "$version" ]; then
  echo "Error: failed to update src/Dolphin/Dolphin.csproj version to $version (found: ${csproj_version:-<missing>})"
  exit 1
fi
